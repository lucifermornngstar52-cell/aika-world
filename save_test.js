...
src += ';globalThis.__T__ = { updateSim, DAY_LEN, saveGame, loadWorld, openWorld, worldsRegistry, genWorld, get villagers(){return villagers}, get stocks(){return stocks}, get simTime(){return simTime}, get objects(){return objects}, get huts(){return huts}, get worldId(){return worldId} };';
eval(src);
...
// сохранить
T.saveGame();
const savedId = T.worldId;
console.log('сейв размер:', (store['aikaWorld_' + savedId] || '').length, 'байт');
// изменить мир (ещё 3 дня) и загрузиться
for (let t = 0; t < T.DAY_LEN * 3; t += 0.25) T.updateSim(0.25);
const simBefore = T.simTime;
const ok = T.loadWorld(savedId);
console.log('ЗАГРУЗКА:', ok ? 'успех' : 'ПРОВАЛ');
// автосейв каждый рассвет перезаписывает слот: загрузка откатывает к последнему рассвету
const rolledBack = T.simTime <= simBefore;
console.log('ПОСЛЕ: pop=' + T.villagers.length, 'wood=' + T.stocks.wood,
  'simT=' + T.simTime.toFixed(1) + ' (перед загрузкой было ' + simBefore.toFixed(1) + ')',
  'откат к автосейву:', rolledBack ? 'ДА ✓' : 'НЕТ ✗');
...
process.exit(err === 0 && ok && rolledBack ? 0 : 1);
